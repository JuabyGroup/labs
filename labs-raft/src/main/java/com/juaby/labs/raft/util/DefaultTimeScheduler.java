...
/**
 * Implementation of {@link com.juaby.labs.raft.util.TimeScheduler} by extending
...
public class DefaultTimeScheduler extends ScheduledThreadPoolExecutor implements TimeScheduler {
     ...
     */
    private static int TIMER_DEFAULT_NUM_THREADS = Runtime.getRuntime().availableProcessors();

    private static int THREADPOOL_SHUTDOWN_WAIT_TIME = 60 * 1000;

    protected static final Logger log = LoggerFactory.getLogger(DefaultTimeScheduler.class);
     ...
     * {@link com.juaby.labs.raft.util.DefaultTimeScheduler.Task#nextInterval()} milliseconds. The task is neve done until nextInterval()
